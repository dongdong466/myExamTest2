...
<script language="javascript">
	timer = window.setInterval("showStartTime();showRemainTime();", 1000); //系统不断调用这两个方法，显示时间
	//此处需要加&nocache="+new Date().getTime()，否则将出现时间不自动走动的情况
	function showStartTime() {
		var loader = new net.AjaxRequest(
				"startExam.do?action=showStartTime&nocache="
						+ new Date().getTime(), deal_s, onerror, "GET");
	}
	function showRemainTime() {
		var loader1 = new net.AjaxRequest(
				"startExam.do?action=showRemainTime&nocache="
	...
	function onerror() {
		window
				.open(
						'../index.jsp',
						'',
						'toolbar,menubar,scrollbars,resizable,status,location,directories,copyhistory,height=768,width=1366');
		window.close();
	}
	function deal_s() {
...
<td width="23%" nowrap height=23 align=right>考试时间：</td>
		<td width="14%" nowrap><font color="#FF0000">10</font>分钟</td>
  ...
  <tr>
    <td height="600" colspan="2" valign="top">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td colspan="2" align="center" class="title"><%=lesson.getLesson(lessonID)%>&nbsp;考试试卷
          </td>
        </tr>
        <tr>
          <td width="64%">&nbsp;</td>
	  ...
	  <html:form action="/manage/startExam.do?action=submitTestPaper&uuid=${uuid}" method="post">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
	    <td>
<table id="single" width="90%"  border="0" cellspacing="0" cellpadding="0" align=center>
  ...
  <tr>
    <td colspan="4" height=23 style="font-size:11pt;">一、单选题（<font color=red>每题<%=s%>分，共40分，选错，不选不得分</font>）</td>
  </tr>
...
<table id="single" width="90%"  border="0" cellspacing="0" cellpadding="0" align=center>
  ...
  <tr>
    <td colspan="4" height=23 style="font-size:11pt;">二、多选题（<font color=red>每题<%=m%>分，共60分，多选，漏选，不选不得分</font>）</td>
  </tr>
